...
Layer2 = full(gallery('tridiag',round((L_2-delta_x)/delta_x),sigmaGDC,1-2*sigmaGDC,sigmaGDC)); %Bulk diffusion parameters
...
A = full(gallery('tridiag',steps_x,sigmaGDC,1-2*sigmaGDC,sigmaGDC)); % Create a tridiagonal basis using the electrolyte values
A(1:L_1/delta_x,1:L_1/delta_x)=Layer1; %integrate the sub-matrix L into the main matrix
A(Length-(L_2-1)/delta_x+2:end,Length-(L_2-1)/delta_x+2:end)=Layer2; %integrate the sub-matrix E into the main matrix

%Set up the part of the matrix at the interface with varying width
for l=L_1:L_1+int_width % Creates a loop which ensures that the tridiagonal matrix at the interface is set up with varying interface width
    A(l,l-1) = sigma_int;
    A(l,l) = 1-2*sigma_int;
    A(l,l+1) = sigma_int;
end

A(L_1+int_width,L_1+1+int_width) = sigmaGDC;
A(L_1+1+int_width,L_1+int_width) = sigma_int; % Manual modification to ensure the matrix is set up correctly
...
Layer2_n = full(gallery('tridiag',round((L_2-delta_x)/delta_x),-sigmaGDC,1+2*sigmaGDC,-sigmaGDC));
Layer2_n(end,end) = 1-sigmaGDC;
Layer2_n(end,end-1) = 2*sigmaGDC;
Layer2_n(end,end-2) = -sigmaGDC;

A_n = full(gallery('tridiag',steps_x,-sigmaGDC,1+2*sigmaGDC,-sigmaGDC));
A_n(1:L_1/delta_x,1:L_1/delta_x)=Layer1_n;
A_n(Length-(L_2-1)/delta_x+2:end,Length-(L_2-1)/delta_x+2:end)=Layer2_n;

for l=L_1:L_1+int_width
    A_n(l,l-1) = -sigma_int;
    A_n(l,l) = 1+2*sigma_int;
    A_n(l,l+1) = -sigma_int;
end

A_n(L_1+int_width,L_1+1+int_width) = -sigmaGDC;
A_n(L_1+1+int_width,L_1+int_width) = -sigma_int;
