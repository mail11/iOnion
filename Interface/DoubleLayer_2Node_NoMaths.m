...
D_2=0.026;  % Define diffusivity of electrolyte material [um^2/s] (GDC)
k=0.000292; % Define surface exchange coefficient of LSCF [um/s]
D_int=.00009; %Define interfacial resistance [s/um]
...
sup(1:L(1)/delta_x-1,1)=sigma_1;
sup(L(1)/delta_x-0:L(1)/delta_x,1)=sigma_int;
sup(L(1)/delta_x+1:end,1)=sigma_2;

% Set up the vector for the central diagonal
s1 = zeros(round(steps_x),1);
s1(1:L(1)/delta_x-1,1)=1-2*sigma_1;
s1(L(1)/delta_x,1)=1-(sigma_int+sigma_1);
s1(L(1)/delta_x+1,1)=1-(sigma_int+sigma_2);
s1(L(1)/delta_x+2:end,1)=1-2*sigma_2;

s1_n = zeros(round(steps_x),1);
s1_n(1:L(1)/delta_x-1,1)=1+2*sigma_1;
s1_n(L(1)/delta_x,1)=1+(sigma_int+sigma_1);
s1_n(L(1)/delta_x+1,1)=1+(sigma_int+sigma_2);
s1_n(L(1)/delta_x+2:end,1)=1+2*sigma_2;
